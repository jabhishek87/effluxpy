...
import effluxpy
import effluxpy.compat as compat
import effluxpy.file as effluxpy_file
import effluxpy.manager as effluxpy_manager
import effluxpy.plugin.player as player
import effluxpy.plugin.player.playable as player_playable
...
class TestIntegrationBase(TestPlayerBase):
    ...
    effluxpy_module = effluxpy
    manager_module = effluxpy_manager
...
class TestIntegration(TestIntegrationBase):
    ...
    def test_register_plugin(self):
        self.app.config.update(self.effluxpy_module.app.config)
        self.app.config['plugin_namespaces'] = ('effluxpy.plugin',)
    ...
    def test_reload(self):
        self.app.config.update(
            ...
            plugin_namespaces=['effluxpy.plugin']
        )
        manager = self.manager_module.PluginManager(self.app)
        manager.load_arguments(self.non_directory_args)
        manager.reload()
...
class TestPlayable(TestIntegrationBase):
    ...
    def test_playabledirectory(self):
        ...
        try:
            ...
            open(file, 'w').close()
            node = effluxpy_file.Directory(tmpdir)
...
class TestBlueprint(TestPlayerBase):
    def setUp(self):
        ...
        self.app = effluxpy.app  # required for our url_for calls
        self.app.config.update(
