...
.. currentmodule:: effluxpy.manager

effluxpy is extensible via a powerful plugin API. A plugin can register
...
A fully functional :mod:`effluxpy.plugin.player` plugin module is provided as
example.
...
Aiming to make plugin names shorter, effluxpy try to load plugins
using namespaces
and prefixes defined on configuration's ``plugin_namespaces`` entry on
:attr:`effluxpy.app.config`. Its default value is effluxpy's built-in module namespace
`effluxpy.plugins`, `effluxpy_` prefix and an empty namespace (so any plugin could
be used with its full module name).

Summarizing, with default configuration:

* Any python module inside `effluxpy.plugin` can be loaded as plugin by its
  relative module name, ie. ``player`` instead of ``effluxpy.plugin.player``.
* Any python module prefixed by ``effluxpy_`` can be loaded as plugin by its
  unprefixed name, ie. ``myplugin`` instead of ``effluxpy_myplugin``.
...
Your built-in plugin, placed under `effluxpy/plugins/` in your own
effluxpy fork:

.. code-block:: bash

  effluxpy --plugin=my_builtin_module

Your prefixed plugin, a regular python module in python's library path,
named `effluxpy_prefixed_plugin`:

.. code-block:: bash

  effluxpy --plugin=prefixed_plugin

Your plugin, a regular python module in python's library path, named `my_plugin`.

.. code-block:: bash

  effluxpy --plugin=my_plugin
...
.. code-block:: python
  ...
  def register_plugin(manager):
      ...
      :type manager: effluxpy.manager.PluginManager
      '''
...
In case you need to add extra command-line-arguments to effluxpy command,
...
:class:`effluxpy.file.Node` (commonly a :class:`effluxpy.file.File` or a
:class:`effluxpy.file.Directory`) instance.
...
Function :func:`effluxpy.file.defaultsnamedtuple` is a
...
a :class:`effluxpy.file.Node` instance as argument and returning said :class:`str`,
...
:meth:`effluxpy.file.Node.from_urlpath` static method to create the
appropriate file/directory object (see :mod:`effluxpy.file`).
...
A good way to keep your plugin working on future effluxpy releases is
:ref:`upstreaming it <builtin-plugins-contributing>` onto effluxpy itself.
